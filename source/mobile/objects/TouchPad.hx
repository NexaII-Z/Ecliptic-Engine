...
class TouchPad extends MobileInputManager implements IMobileControls
{
	...
	public function new(DPad:String, Action:String, ?Extra:ExtraActions = NONE)
	{
		...
		if (DPad != "NONE") {
			if (!MobileData.dpadModes.exists(DPad)) throw 'TouchPad dpadMode "$DPad" not found.';
			for (bd in MobileData.dpadModes.get(DPad).buttons) {
				Reflect.setField(this, bd.button, createButton(bd.x, bd.y, bd.graphic, CoolUtil.colorFromString(bd.color), Reflect.getProperty(this, bd.button).IDs));
				add(Reflect.field(this, bd.button));
			}
		}

		if (Action != "NONE") {
			if (!MobileData.actionModes.exists(Action)) throw 'TouchPad actionMode "$Action" not found.';
			for (bd in MobileData.actionModes.get(Action).buttons) {
				Reflect.setField(this, bd.button, createButton(bd.x, bd.y, bd.graphic, CoolUtil.colorFromString(bd.color), Reflect.getProperty(this, bd.button).IDs));
				add(Reflect.field(this, bd.button));
			}
		}
	...
	public function setExtrasDefaultPos() {
		var int:Int = 0;
		if (MobileData.save.data.extraData == null) MobileData.save.data.extraData = new Array();
	...
	public function setExtrasPos() {
		var int:Int = 0;
		if (MobileData.save.data.extraData == null) setExtrasDefaultPos();
		for (button in Reflect.fields(this)) {
			var field = Reflect.field(this, button);
			if (button.toLowerCase().contains('extra') && Std.isOfType(field, TouchButton)) {
				if (MobileData.save.data.extraData.length > int) setExtrasDefaultPos();
	...
	private function createButton(X:Float, Y:Float, Graphic:String, ?Color:FlxColor = 0xFFFFFF, ?IDs:Array<MobileInputID>):TouchButton {
		...
		// Ecliptic path: assets/Mains/mobile/images/touchpad/
		var bgPath   = 'assets/Mains/mobile/images/bg.png';
		var iconPath = 'assets/Mains/mobile/images/touchpad/${Graphic.toUpperCase()}.png';
		if (openfl.utils.Assets.exists(bgPath))   button.loadGraphic(bgPath);
		if (openfl.utils.Assets.exists(iconPath)) button.label.loadGraphic(iconPath);
